...
import { MoonIcon } from "@/components/common/CustomIcons";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function Navbar({ onOpenMobileMenu }: { onOpenMobileMenu: () => void }) {
  ...
  return (
    <header className="flex items-center justify-between h-14 sm:h-16 px-3 sm:px-6 bg-white dark:bg-background border-b border-slate-200 dark:border-slate-700 flex-shrink-0 gap-2">
      {/* Left: hamburger */}
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
        ...
        <Button
          ...
          size="icon"
          onClick={toggleTheme}
          className="h-11 w-11 rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-background dark:text-slate-300 dark:hover:bg-slate-600"
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <img
                ...
                alt="Avatar"
                className="h-11 w-11 aspect-square rounded-lg object-cover flex-shrink-0 scale-125 bg-slate-200 dark:bg-background"
              />
            </Button>
          </DropdownMenuTrigger>
