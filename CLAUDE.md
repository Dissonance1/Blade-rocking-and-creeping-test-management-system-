...
- **Hardware bridges:** `weighing_bridge.py`, `dti_bridge.py`, and `oak1_camera_service.py` are standalone processes outside Docker Compose. Register as Windows Scheduled Tasks via `scripts/register_bridge_tasks.ps1`. `dti_bridge.py` only applies to a DTI gauge that's genuinely classic Bluetooth SPP (or wired RS-232) — the Sylvac S_Dial WORK gauge currently in use is BLE-only and instead runs in Bluetooth HID keyboard mode with a frontend-side guard; see "DTI gauge (Sylvac S_Dial WORK / SY289)" under Secondary Hardware Stations for why and how.
...
### DTI gauge (Sylvac S_Dial WORK / "SY289") — Bluetooth HID keyboard mode, NOT `dti_bridge.py`

The Sylvac S_Dial WORK gauge (manual: `MAN-S_DIAL-WORK-AD-BT-NA-NEU_681.289.01-120_WEB.pdf`, product code 681.289) is **Bluetooth 4.0 (BLE) only** — confirmed empirically: removing and re-pairing it in Windows Bluetooth settings connects instantly with no PIN prompt, which only happens for BLE pairing, never classic Bluetooth SPP. This means it can **never** bind to a raw Windows COM port the way `weighing_bridge.py`'s scale does (that gauge/adapter genuinely is classic Bluetooth SPP, hardcoded to COM3) — `dti_bridge.py`'s raw-serial approach (`--port COMx`) does not and cannot work for this gauge model over Bluetooth, regardless of which COM port is guessed.

Two ways were evaluated to get readings off this gauge; **the current deployed setup uses the second one, not the first:**

1. **VMUX2** (`C:\Program Files\VMUX2`, Sylvac's own official BLE→virtual-COM-port bridge app, bundles Electronic Team's Virtual Serial Port Driver Pro). This *can* work — it was gotten running once, bridging the gauge to COM4 with `dti_bridge.py --port COM4` reading it successfully, preserving the whole original architecture (station tagging, WebSocket auto-fill, debounce). But it's fragile in practice: the gauge's own on-device Bluetooth menu (`bt.CFG`, reached via MODE long-press → short-presses to `bt.CFG`) has three profiles — `SIMPLE` (what VMUX2 needs), `PAIr`, and `HId` (HID keyboard emulation) — and **switching profiles, or using `bt` → `rESEt` on the gauge, wipes its Bluetooth pairing info** (`C:\ProgramData\Sylvac\VMUX2\Configuration\new_configuration.json`'s `Instruments` array goes back to `[]`). Re-adding it must be done through VMUX2's own "add instrument" scan — not Windows' Bluetooth pairing UI, which only re-adds it as a bare BLE device with no virtual COM port. Windows also caches a BLE device's GATT/HID table per-bond, so if the gauge was ever paired as `HId`, you must fully **remove/forget the device in Windows Settings → Bluetooth & devices** (not just switch the gauge's profile back) before VMUX2 or a fresh scan will see the correct service table again.
2. **Bluetooth HID keyboard mode (what's actually running today).** Set the gauge's profile to `HId` (MODE long-press → `bt.CFG` → favorite button cycles `SIMPLE`/`PAIr`/`HId` → `SET` to confirm), then pair it in Windows like any Bluetooth keyboard (Settings → Bluetooth & devices → Add device → Bluetooth — pairs instantly, no PIN, no driver). Each reading then "types" itself as raw keystrokes (e.g. `+0002.89`) into whatever has OS-level focus — no VMUX2, no COM port, no `dti_bridge.py`, no `--station` flag, no `hardware_station` provenance tagging. This trades the old architecture's robustness for zero extra software.

**Frontend guard for HID mode** (since raw HID keystrokes go to *whatever has focus*, not to a scoped data channel):
- `frontend/src/hooks/useHidGaugeAppGuard.ts`, mounted at the true app root (`App.tsx`, ahead of routing — covers the login page too): detects gauge output by inter-keystroke burst timing (<35ms apart — reliably faster than human typing) and blocks/swallows it everywhere **except** the `/rocking-creep` route, so a stray SEND press can't corrupt an unrelated field elsewhere in the app.
- `frontend/src/pages/RockingCreepPage.tsx` owns its own capture-and-route guard for that one page: every detected burst — **even one that lands on an already-focused target input** (the page auto-focuses the active target field, so naive keystrokes would otherwise type straight in unformatted) — is intercepted, parsed, rounded to 2 decimals (mm, `0.00` format), and applied via `applyReading()` to whichever Rocking/Creep cell is the current `activeTarget`, exactly like a WebSocket-delivered reading from the old bridge path would be.
- **Known unresolved limitation:** this is page/JS-level protection only — it works exclusively while the browser tab itself has actual Windows window focus. If OS focus is on a different application/window entirely when SEND is pressed, the reading types there unguarded; no web page code can intercept that. A true fix needs OS-level, per-device keystroke interception (Windows already binds this gauge as a **Keyboard-class device** via `kbdhid.sys`, which rules out both Raw Input alone — can identify the source device but can't suppress delivery — and reading the HID collection directly via hidapi/pywinusb — the class driver has already claimed it exclusively). The only reliable route identified is the third-party **Interception** kernel driver (https://github.com/oblitum/Interception), which is what a from-scratch background bridge script would need to build on to suppress only the gauge's keystrokes system-wide and re-post them to the backend like the old `dti_bridge.py` did — **not yet implemented** as of this writing.

To replicate this on a secondary station PC (e.g. the HPTR PC): pair its own gauge in `HId` mode the same way, and make sure the frontend build it's using includes the two files above (it will, since it's the one central frontend build served to every station — see "Do, on the second PC" step 4). No `-Station`/`--station` value applies to DTI readings under this approach, since there's no bridge script tagging them.
...
`hptr_ocr_service.py` also loads its own, completely separate recognizer/detector checkpoint — `backend/app/ocr/models/ppocrv4-hptr/` — via `PaddleOCRProvider(models_dir=...)`, never the OH backend's `backend/app/ocr/models/ppocrv4/`. The two directories started out byte-identical (one was copied from the other) but are never the same files on disk, and `PaddleOCRProvider`'s engine cache is keyed by resolved `models_dir` so the two never share a loaded engine instance even within the same process. This exists so a future HPTR-only fine-tune (real captures from this station only) can overwrite `ppocrv4-hptr/` without ever touching, retraining, or reloading the OH model — see `backend/finetune/README.md` before running any training and confirm the target checkpoint is `ppocrv4-hptr/`, not `ppocrv4/`, before deploying anything.
