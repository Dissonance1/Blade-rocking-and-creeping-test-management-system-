"""
One-off import of the real HPTR_16-96-317.xls dataset into a new Work Order.

Reads the parsed JSON (see scratchpad/parse_hptr_317.py) rather than the .xls
...
Scope: serial number, melt number, and weight data only. Slot allocation and
rocking values are entered separately later through the normal Assembly /
Rocking & Creep Entry workflows, so blades are left at OH_INSPECTION with no
SlotAllocation rows and rocking_value=None.
...
and all backend dependencies)::
    ...
    docker exec oh_backend python scripts/import_hptr_16_96_317.py
"""
...
PARSED_JSON = Path(__file__).resolve().parent.parent / "hptr_16_96_317_parsed.json"
...
async def _get_or_create_work_order(db: AsyncSession, created_by: User) -> WorkOrder:
    ...
    db.add(work_order)
    await db.flush()
    _log_created(f"WorkOrder {WORK_ORDER_NUMBER}  (HPTR, 90 blades, from HPTR_16-96-317.xls)")
    return work_order
...
async def import_hptr() -> None:
    if not PARSED_JSON.exists():
        raise SystemExit(f"Missing {PARSED_JSON} — parse the .xls first")
    ...
    print("Scope: serial number, melt number, weight/static moment only.")
